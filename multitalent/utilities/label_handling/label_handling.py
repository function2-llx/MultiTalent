...
from multitalent.utilities.helpers import softmax_helper_dim0
...
class LabelManagerForceUseLabels(LabelManager):
    def __init__(self, label_dict: dict, regions_class_order: Union[List[int], None], inference_nonlin=None):
        super().__init__(label_dict, regions_class_order, True, inference_nonlin)
...
def get_labelmanager_class_from_plans(plans: dict) -> Type[LabelManager]:
    if 'label_manager' not in plans.keys():
        ...
        labelmanager_class = recursive_find_python_class(join(multitalent.__path__[0], "utilities", "label_handling"),
                                                         plans['label_manager'],
                                                         current_module="multitalent.utilities.label_handling")
