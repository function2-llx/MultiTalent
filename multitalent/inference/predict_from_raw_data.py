...
import multitalent
from multitalent.configuration import default_num_processes
from multitalent.inference.data_iterators import PreprocessAdapterFromNpy, preprocessing_iterator_fromfiles, \
...
from multitalent.utilities.file_path_utilities import get_output_folder, check_workers_alive_and_busy
...
from multitalent.utilities.json_export import recursive_fix_for_json_export
...
from multitalent.utilities.utils import create_lists_from_splitted_dataset_folder
...
class nnUNetPredictor(object):
    ...
    def initialize_from_trained_model_folder(self, model_training_output_dir: str,
        ...
        trainer_class = recursive_find_python_class(join(multitalent.__path__[0], "training", "nnUNetTrainer"),
                                                    trainer_name, 'multitalent.training.nnUNetTrainer')
        if trainer_class is None:
            raise RuntimeError(f'Unable to locate trainer class {trainer_name} in multitalent.training.nnUNetTrainer. '
...
def predict_entry_point_modelfolder():
    ...
    parser.add_argument('-device', type=str, default='cuda', required=False,
                        help="Use this to set the device the inference should run with. Available options are 'cuda' "
                             ...
                             "Use CUDA_VISIBLE_DEVICES=X multitalent_predict [...] instead!")
...
def predict_entry_point():
    ...
    parser.add_argument('-num_parts', type=int, required=False, default=1,
                        help='Number of separate multitalent_predict call that you will be making. Default: 1 (= this one '
                             'call predicts everything)')
    parser.add_argument('-part_id', type=int, required=False, default=0,
                        help='If multiple multitalent_predict exist, which one is this? IDs start with 0 can end with '
                             'num_parts - 1. So when you submit 5 multitalent_predict calls you need to set -num_parts '
    ...
    assert args.part_id < args.num_parts, 'Do you even read the documentation? See multitalent_predict -h.'
...
if __name__ == '__main__':
    ...
    from multitalent.paths import nnUNet_results, nnUNet_raw
    ...
    from multitalent.imageio.simpleitk_reader_writer import SimpleITKIO
